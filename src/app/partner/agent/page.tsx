...
export default function AgentPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetchApi(
        `/api/v1/partner/agent?page=${page}&page_size=${pageSize}&keyword=${keyword}`
      );
      if (res.code === 0) {
        setData(res.data);
      } else {
  ...
  const handleSubmit = async () => {
    if (!form.usertel || !form.contact) {
      toast.error('请填写必填字段');
      return;
    }
    setSubmitting(true);
    try {
      const url = editing ? `/api/v1/partner/agent/${editing.id}` : '/api/v1/partner/agent';
  ...
  const handleStatusUpdate = async (id: number, status: number) => {
    try {
      const res = await fetchApi(`/api/v1/partner/agent/${id}/status`, {
        method: 'PUT',
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">地区代理管理</CardTitle>
        </CardHeader>
        <CardContent>
          {/* Search & Actions */}
                ...
                {loading ? (
                  ...
                  data.list.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="font-medium">{item.usertel}</TableCell>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                value={form.uno}
                onChange={(e) => updateForm('uno', e.target.value)}
              />
            </div>
            <div className="flex items-center gap-3 sm:col-span-2">
              <div className="flex items-center gap-2">
                <Switch
                  checked={form.sms === 1}
                  onCheckedChange={(checked) => updateForm('sms', checked ? 1 : 0)}
                />
                <Label>短信通知</Label>
              </div>
              <div className="flex items-center gap-2">
                <Switch
                  ...
                  onCheckedChange={(checked) => updateForm('display', checked ? 1 : 0)}
                />
                <Label>显示</Label>
              </div>
            </div>
