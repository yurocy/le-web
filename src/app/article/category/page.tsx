...
type FormData = Omit<NewsCategory, 'id' | 'created_at' | 'updated_at'>;
...
export default function ArticleCategoryPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetchApi('/api/v1/article/category');
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        await fetchApi(`/api/v1/article/category/${editingItem.id}`, {
          method: 'PUT',
          body: JSON.stringify(form),
        });
      } else {
        await fetchApi('/api/v1/article/category', {
          method: 'POST',
          body: JSON.stringify(form),
        });
      }
  ...
  const handleDelete = async (id: number) => {
    try {
      await fetchApi(`/api/v1/article/category/${id}`, { method: 'DELETE' });
  ...
  const toggleStatus = async (item: NewsCategory) => {
    ...
    try {
      await fetchApi(`/api/v1/article/category/${item.id}`, {
        method: 'PUT',
    ...
    <div className="min-h-screen bg-background p-4 md:p-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-xl">文章分类管理</CardTitle>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingItem ? '修改文章分类信息' : '填写新文章分类信息'}
            </DialogDescription>
          </DialogHeader>
