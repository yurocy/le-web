...
type FormData = Omit<PricingCategory, 'id' | 'created_at' | 'updated_at'>;
...
export default function PricingCategoryPage() {
  ...
  const [editingItem, setEditingItem] = useState<PricingCategory | null>(null);
  ...
  const openEdit = (item: PricingCategory) => {
    setEditingItem(item);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        await fetchApi(`/api/v1/pricing/category/${editingItem.id}`, {
          method: 'PUT',
          body: JSON.stringify(form),
        });
      } else {
        await fetchApi('/api/v1/pricing/category', {
          method: 'POST',
          body: JSON.stringify(form),
        });
      }
  ...
  const handleDelete = async (id: number) => {
    try {
      await fetchApi(`/api/v1/pricing/category/${id}`, { method: 'DELETE' });
  ...
  const toggleStatus = async (item: PricingCategory) => {
    const newStatus = item.status === 1 ? 0 : 1;
    try {
      await fetchApi(`/api/v1/pricing/category/${item.id}`, {
        method: 'PUT',
    ...
    <div className="min-h-screen bg-background p-4 md:p-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-xl">报价分类管理</CardTitle>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingItem ? '修改报价分类信息' : '填写新报价分类信息'}
            </DialogDescription>
          </DialogHeader>
